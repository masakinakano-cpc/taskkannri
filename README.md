...
- ✅ **外部サービス連携**: Chatwork、Google Chat、Gmailからメッセージを自動取得してタスク化
...
#### 0.1 対応サービス
**概要**: Chatwork、Google Chat、Gmailから自動的にメッセージを取得してタスク化

**機能詳細**:
- **Chatwork連携**
  - Chatwork APIを使用してルームのメッセージを取得
  - ルーム単位・キーワード・送信者でフィルタリング
  - メッセージを自動的にタスクに変換
...
#### 0.2 同期ルール設定
**概要**: メッセージをタスクに変換するための詳細なルール設定

**機能詳細**:
- **フィルター設定**
  - キーワードフィルター（カンマ区切りで複数指定）
  - 送信者フィルター
  - ラベルフィルター（Gmailのみ）
  - ルーム/スペースフィルター

- **自動タスク化**
  - ルールに一致したメッセージを自動的にタスクとして作成
  - デフォルト優先度の設定
  - デフォルト担当者の設定
...
詳細は [EXTERNAL_INTEGRATION.md](./EXTERNAL_INTEGRATION.md) を参照してください。
...
### Phase 2（短期）
- [x] 外部サービス連携機能（Chatwork、Google Chat、Gmail）
